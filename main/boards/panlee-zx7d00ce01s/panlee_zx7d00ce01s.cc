...
private:
    ...
    void InitializeIoExpander() {
        ...
        // If a ZXACC-EXTSG-V12 speaker board is plugged into the extension
        // header, its NS4168 amp's MUTE/channel-select line is wired here.
        // Driven high to enable output; if the speaker stays silent (or
        // only passes distorted/attenuated audio), try driving this low
        // instead - the silkscreen labels it "MUTE" but electrically it's
        // the NS4168's L/R CTRL pin, and its correct idle level for this
        // board isn't documented anywhere public.
        esp_io_expander_set_dir(io_expander_, AW9523_SPK_MUTE_PIN, IO_EXPANDER_OUTPUT);
        esp_io_expander_set_level(io_expander_, AW9523_SPK_MUTE_PIN, 1);
    }
...
public:
    ...
    virtual AudioCodec* GetAudioCodec() override {
        // Speaker side: ZXACC-EXTSG-V12 (NS4168 I2S amp), pins per its
        // silkscreen - see config.h. Mic side: a separate external I2S
        // mic is assumed; update AUDIO_I2S_MIC_* in config.h to match
        // your actual mic wiring if it differs.
        //
        // I2S_STD_SLOT_BOTH on the speaker side duplicates the mono audio
        // into both the left and right I2S slots. The NS4168's CTRL pin
        // (wired to AW9523 P07 above) picks which single slot it actually
        // listens to, and we don't have a reliable way to know which
        // channel "driven high" maps to on this board - sending audio on
        // both slots means it doesn't matter, instead of getting total
        // silence if CTRL ends up selecting the slot that's never written.
        static NoAudioCodecSimplex audio_codec(
            AUDIO_INPUT_SAMPLE_RATE, AUDIO_OUTPUT_SAMPLE_RATE,
            AUDIO_I2S_SPK_GPIO_BCLK, AUDIO_I2S_SPK_GPIO_WS, AUDIO_I2S_SPK_GPIO_DOUT, I2S_STD_SLOT_BOTH,
